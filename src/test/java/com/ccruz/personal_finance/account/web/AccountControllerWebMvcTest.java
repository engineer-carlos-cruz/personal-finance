...
class AccountControllerWebMvcTest {
    ...
    @BeforeEach
    void setUp() {
        Mockito.reset(accountService, accountMapper);
    }
    ...
    @Test
    @DisplayName("create should return 400 when code is blank")
    void create_shouldReturn400WhenCodeIsBlank() throws Exception {
        mockMvc.perform(post("/api/accounts")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"code\":\"\",\"description\":\"Cash account\",\"balance\":1000.00}"))
                .andExpect(status().isBadRequest());
    }

    @Test
    @DisplayName("create should return 400 when balance is negative")
    void create_shouldReturn400WhenBalanceIsNegative() throws Exception {
        mockMvc.perform(post("/api/accounts")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"code\":\"CASH\",\"description\":\"Cash account\",\"balance\":-100.00}"))
                .andExpect(status().isBadRequest());
    }
    ...
    @Test
    @DisplayName("update should return 400 when description is blank")
    void update_shouldReturn400WhenDescriptionIsBlank() throws Exception {
        mockMvc.perform(put("/api/accounts/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"code\":\"CASH\",\"description\":\"\",\"balance\":1000.00}"))
                .andExpect(status().isBadRequest());
    }
